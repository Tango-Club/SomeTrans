...
const std::unordered_map<folly::fbstring, ValueType> TypeMP{
...
struct RowData
{
    std::vector<std::variant<int, long long, unsigned long long, folly::fbstring, float, double>> RowValue;
};
struct ColumnDefType
{ //数据类型
    std::vector<int> args;
    ValueType type;
    ColumnDefType(const folly::fbstring &typeStr)
    {
        folly::fbstring typeName;
        int pre = 0;
        for (size_t i = 0; i < typeStr.length(); i++)
        {
            ...
            {
                this->args.push_back(std::stoi(typeStr.substr(pre, i - pre).toStdString()));
                pre = i + 1;
            }
        }
        if (pre == 0)
...
bool isInteger(const folly::fbstring &s)
{
    int cnt = 0;
    for (auto &i : s)
    {
...
bool isDecimal(const folly::fbstring &s)
{
    int cnt = 0;
    int cnt1 = 0;
    for (auto &i : s)
...
{ //列声明
    ...
    std::variant<int, long long, unsigned long long, folly::fbstring, float, double> readCol(folly::fbstring &data)
    {
        if (this->columnDef.type == ValueType::Vtinyint)
        {
			...
			*/
            ...
            try
            {
                int x = std::stoi(data.toStdString());
                if (!this->isUnsigned)
                {
                    if (x < -128 || x > 127)
                        return 0;
		...
		*/
        if (this->columnDef.type == ValueType::Vint)
        {
			...
			*/
            ...
            try
            {
                long long x = std::stoll(data.toStdString());
                if (!this->isUnsigned)
                {
        ...
        if (this->columnDef.type == ValueType::Vbigint)
        {
			...
			*/
            ...
            try
            {
                ...
                {
                    unsigned long long x = std::stoull(data.toStdString());
                    return x;
                }
            }
        ...
        if (this->columnDef.type == ValueType::Vdecimal)
        {
			...
			*/
            if (isDecimal(data))
            {
                try
                {
                    double ret = std::stod(data.toStdString());
                    return ret;
                }
                catch (const std::exception &)
                {
		...
		*/
        if (this->columnDef.type == ValueType::Vdatetime)
        {
			...
			*/
            ...
            /*folly::fbstring regex_template("\\d{4}[-]\\d{2}[-]\\d{2}[ ]\\d{2}[:]\\d{2}[:]\\d{2}[.]\\d{1,3}");
			std::regex pattern(regex_template, std::regex::icase);
			std::match_results<folly::fbstring::const_iterator> result;
...
{ //索引声明
    //临时
    folly::fbstring str;
    IndexInfo(const folly::fbstring &indexStr)
    {
        this->str = indexStr;
    }
};
...
struct TableInfo
{ //表声明
    folly::fbstring tableName;
    folly::fbstring fromDataBase;
    ...
    void readRow(std::vector<folly::fbstring> &vecStr)
    {
        RowData rowData;
        for (size_t i = 0; i < columns.size(); i++)
        {
    ...
    void sink(folly::fbstring path)
    {
        ...
        remove(path.c_str());
        std::ofstream dataSink(path.toStdString());
        dataSink.tie(0);
        size_t rNums = 0;
        for (auto &row : datas)
        {
            ...
            for (auto &value : row.RowValue)
            {
                if (f)
                    ...
                    dataSink << "	";
                if (auto pval = std::get_if<folly::fbstring>(&value))
                    dataSink << *pval;
